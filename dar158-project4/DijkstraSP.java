...
public class DijkstraSP {
    ...
    private DirectedDiWeightEdge[] edgeTo;    // edgeTo[v] = last edge on shortest s->v path
     ...
     * @param isPrice true for priced based shortest path false for distance
     * based
     * @throws IllegalArgumentException if an edge weight is negative
     * @throws IllegalArgumentException unless {@code 0 <= s < V}
     */
    public DijkstraSP(EdgeWeightedDigraph G, int s, boolean isPrice) {
        double weight;
        for (DirectedDiWeightEdge e : G.edges()) {
            if (isPrice) {
                weight = e.price();
            } else {
                weight = e.distance();
        ...
        edgeTo = new DirectedDiWeightEdge[G.V()];
        for (int v = 0; v < G.V(); v++) {
...
//        assert check(G, s);
    ...
    private void relax(DirectedDiWeightEdge e, boolean isPrice) {
        double weight;
        if (isPrice) {
            weight = e.price();
        } else {
     ...
     */
    public Iterable<DirectedDiWeightEdge> pathTo(int v) {
        if (!hasPathTo(v)) {
            return null;
        }
        Stack<DirectedDiWeightEdge> path = new Stack<DirectedDiWeightEdge>();
        for (DirectedDiWeightEdge e = edgeTo[v]; e != null; e = edgeTo[e.from()]) {
            path.push(e);
        }
        return path;
    }
